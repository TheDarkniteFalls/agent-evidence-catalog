...
Publication status: [Research Preview v0.1](https://thedarknitefalls.github.io/agent-evidence-catalog/) is the public static preview. The prepared 2026-08-17 refresh candidate is a sealed official-source review snapshot covering 55 defensible agent surfaces: 53 records current within the snapshot and 48 non-current records, including 45 superseded identities, for 101 records total. Three official-source exact-identity successors were added to the accepted 98-record projection while every predecessor remains inspectable: Cursor 3.16, Qwen Code CLI 0.21.13 and Junie IDE plugin 262.579.20. The exact review window and seal time are derived from the retained audit receipts, and a separate one-shot publication freshness census reports known-newer identities and incomplete coverage without promoting either into the snapshot. Thirty-nine breadth additions remain bounded to official identity and delivery claims, keep their applicability gaps unknown and add no independent evidence. The prepared candidate does not itself update the live site; publication remains a separate authorized action. An operational private reporting route is deferred to `ROADMAP.md` and is required before sensitive evidence or open intake, not before this static public-source preview. Real-agent intake is not open, and no review or response is promised.
