...
Publication status: [Research Preview v0.1](https://thedarknitefalls.github.io/agent-evidence-catalog/) is the public static preview. The repository's latest prepared release is built from the sealed 2026-08-20 official-source review snapshot covering 55 defensible agent surfaces: 53 records current within the snapshot and 62 non-current records, including 59 superseded identities, for 115 records total. Nine exact-identity successors were added to the accepted 106-record projection while every predecessor remains inspectable: Qwen Code CLI 0.21.14, Kiro IDE 1.0.337, OpenCode CLI 1.18.19, Claude Code CLI 2.1.237, Antigravity CLI 1.1.16, Gemini CLI 0.56.0, Junie IDE plugin 262.579.38, OpenAI Codex CLI 0.148.0 and Zed Agent 1.16.1. The exact review window and seal time are derived from retained audit receipts, and a separate one-shot publication freshness census reports known-newer identities and incomplete coverage without promoting either into the snapshot. A prepared or committed candidate does not update the public URL by itself; the Pages deployment and live Browser receipts establish which exact candidate it serves. Four additional products—Cursor CLI, Cascade in Windsurf IDE, Copilot Agent Mode for Visual Studio and Zoo Code v3.78.0—remain source-only dossiers outside catalog, mapping, lifecycle and presentation admission. Thirty-nine earlier breadth additions remain bounded to official identity and delivery claims, keep their applicability gaps unknown and add no independent evidence. An operational private reporting route is deferred to `ROADMAP.md` and is required before sensitive evidence or open intake, not before this static public-source preview. Real-agent intake is not open, and no review or response is promised.
...
- `drafts/research-preview-release/currentness-2026-08-20/` — the latest all-surface refresh workflow, nine additive exact-identity successors, 46 retained surface decisions, preferred-source and full-corpus URL receipts, exact snapshot seal, one-shot publication freshness census and dated currentness receipt.
