...
Publication status: [Research Preview v0.1](https://thedarknitefalls.github.io/agent-evidence-catalog/) is the public static preview. This release is built from the sealed 2026-08-18 official-source review snapshot covering 55 defensible agent surfaces: 53 records current within the snapshot and 53 non-current records, including 50 superseded identities, for 106 records total. Five exact-identity successors were added to the accepted 101-record projection while every predecessor remains inspectable: Claude Code CLI 2.1.234, Antigravity CLI 1.1.14, Junie IDE plugin 262.579.25, GitLab Duo Developer Flow 19.2.4-ee and GitLab Code Review Flow 19.2.4-ee. The exact review window and seal time are derived from retained audit receipts, and a separate one-shot publication freshness census reports known-newer identities and incomplete coverage without promoting either into the snapshot. Four additional products—Cursor CLI, Cascade in Windsurf IDE, Copilot Agent Mode for Visual Studio and Zoo Code v3.78.0—remain source-only dossiers outside catalog, mapping, lifecycle and presentation admission. Thirty-nine earlier breadth additions remain bounded to official identity and delivery claims, keep their applicability gaps unknown and add no independent evidence. An operational private reporting route is deferred to `ROADMAP.md` and is required before sensitive evidence or open intake, not before this static public-source preview. Real-agent intake is not open, and no review or response is promised.
...
- `drafts/research-preview-release/currentness-2026-08-18/` — the latest all-surface refresh workflow, five additive exact-identity successors, 50 unchanged surface decisions, preferred-source and full-corpus URL receipts, exact snapshot seal, one-shot publication freshness census and dated currentness receipt.
