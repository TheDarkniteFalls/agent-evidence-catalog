...
assert.equal(audit.asOf, "2026-08-17");
assert.equal(audit.counts.recordsChecked, 101);
...
source.sourceLinkAudit = {
  ...
  method: "HTTP GET with redirects against every unique named official source URL in the projected 101-record corpus",
...
console.log(`PASS bound ${audit.counts.uniqueOfficialUrlsChecked} projected official URLs to the 101-record currentness source (${audit.counts.unreachable} unresolved)`);
