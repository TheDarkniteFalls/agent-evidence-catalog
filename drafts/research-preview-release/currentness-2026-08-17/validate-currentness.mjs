...
const snapshotSeal = await readJson("drafts/research-preview-release/currentness-2026-08-17/snapshot-seal.json");
...
assert.equal(source.asOf, "2026-08-17");
assert.equal(surfaceAudit.asOf, "2026-08-17");
...
assert.equal(source.sourceLinkAudit.recordsChecked, 101);
...
assert.equal(source.transitions.length, 3);
...
assert.equal(preview.provenance.currentness20260817Sha256, sha256(sourceText));
assert.equal(preview.provenance.currentness20260817OfficialSourceAuditSha256, sha256(surfaceAuditText));
assert.equal(preview.provenance.currentness20260817OfficialUrlAuditSha256, sha256(urlAuditText));
...
assert.equal(preview.counts.recordsPresentedIncludingHistory, 101);
...
assert.equal(new Set(preview.previewRecords.map((record) => record.recordId)).size, 101);
assert.equal(preview.surfaces.flatMap((surface) => surface.history).length, 48);

assert.deepEqual(snapshotSeal, preview.snapshotSeal);
assert.equal(snapshotSeal.asOf, "2026-08-17");
...
assert.equal(cursorFreshness.reviewedIdentity, "3.16");
assert.equal(cursorFreshness.knownNewerIdentity, null);
assert(cursorFreshness.evidence.detectedVersions.includes("3.16"));
...
assert.equal(lifecycle.entries.filter((entry) => entry.status === "superseded").length, 45);
...
assert(lifecycle.entries.every((entry) => entry.reviewedAt === "2026-08-17"));
...
for (const transition of source.transitions) {
  ...
  assert.equal(summary.reviewedAt, transition.reviewedAt ?? "2026-08-17");
  ...
  for (const claim of record.claims) {
    ...
    assert.equal(raw.review.reviewedAt, transition.reviewedAt ?? "2026-08-17");
    assert.equal(raw.review.recheckAfter, transition.recheckAfter ?? "2026-09-17");
    assert.equal(raw.source.capturedAt, transition.checkedAt ?? "2026-08-17T00:00:00.000Z");
...
for (const surfaceKey of source.unchangedSurfaceKeys) {
  ...
  assert(records.every((record) => record.reviewedAt === "2026-08-17"));
}
...
assert.equal(receipt.scope.priorRecordsRetained, 98);
assert.equal(receipt.scope.exactIdentitySuccessorsAdded, 3);
assert.equal(receipt.scope.recordsPresentedIncludingHistory, 101);
...
console.log("PASS all 55 accepted surfaces retain their 2026-08-17 official-source decision");
console.log("PASS publication freshness chronology rejects a census that begins before the sealed snapshot");
console.log("PASS 3 exact-identity successors preserve all 98 predecessor records with reciprocal lifecycle links");
console.log("PASS 101 projected records contain 53 current and 48 retained-history records with zero independent-test credit");
