...
assert.equal(source.asOf, "2026-08-15");
assert.equal(surfaceAudit.asOf, "2026-08-15");
assert.equal(surfaceAudit.observations.length, 55);
assert.equal(surfaceAudit.counts.rendered, 55);
assert.equal(surfaceAudit.counts.failed, 0);
assert(surfaceAudit.observations.every((item) => item.result === "rendered"));
assert.equal(source.sourceLinkAudit.recordsChecked, 98);
...
assert.equal(source.transitions.length, 10);
...
assert.equal(preview.provenance.currentness20260815Sha256, sha256(sourceText));
assert.equal(preview.provenance.currentness20260815OfficialSourceAuditSha256, sha256(surfaceAuditText));
assert.equal(preview.provenance.currentness20260815OfficialUrlAuditSha256, sha256(urlAuditText));
...
assert.equal(new Set(preview.previewRecords.map((record) => record.recordId)).size, 98);
...
assert.equal(snapshotSeal.asOf, "2026-08-15");
...
assert.equal(cursorFreshness.status, "known-newer");
assert(/^3\.16(?:\.0)?$/.test(cursorFreshness.knownNewerIdentity), `Expected official Cursor 3.16 notice, got ${cursorFreshness.knownNewerIdentity}`);
...
assert.equal(lifecycle.entries.filter((entry) => entry.status === "superseded").length, 42);
...
assert(lifecycle.entries.every((entry) => entry.reviewedAt === "2026-08-15"));
...
for (const transition of source.transitions) {
  ...
  assert.equal(summary.reviewedAt, transition.reviewedAt ?? "2026-08-15");
  ...
  for (const claim of record.claims) {
    ...
    assert.equal(raw.review.reviewedAt, transition.reviewedAt ?? "2026-08-15");
    assert.equal(raw.review.recheckAfter, transition.recheckAfter ?? "2026-09-15");
    assert.equal(raw.source.capturedAt, transition.checkedAt ?? "2026-08-14T22:20:44.000Z");
...
for (const surfaceKey of source.unchangedSurfaceKeys) {
  ...
  assert(records.every((record) => record.reviewedAt === "2026-08-15"));
}
...
assert.equal(receipt.scope.priorRecordsRetained, 88);
assert.equal(receipt.scope.exactIdentitySuccessorsAdded, 10);
assert.equal(receipt.scope.recordsPresentedIncludingHistory, 98);
...
assert.equal(receipt.officialSurfaceAudit.rendered, 55);
...
assert(receipt.reviewedSurfaces.every((item) => item.officialSource && item.checkedAt && item.renderedAt));
assert.equal(receipt.materialTransitions.length, 10);
assert.equal(receipt.scope.independentEvidenceCredit, 0);

console.log("PASS all 55 accepted surfaces retain their 2026-08-15 official-source decision");
console.log("PASS publication freshness chronology rejects a census that begins before the sealed snapshot");
console.log("PASS 10 exact-identity successors preserve all 88 predecessor records with reciprocal lifecycle links");
console.log("PASS 98 projected records contain 53 current and 45 retained-history records with zero independent-test credit");
