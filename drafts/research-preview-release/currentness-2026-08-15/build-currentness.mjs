...
const initialReviewedAt = "2026-08-15";
const initialCheckedAt = "2026-08-14T22:20:44.000Z";
...
async function buildSuccessor(predecessor, transition, sourceDigest) {
  ...
  record.sourceDossier = {
    path: "drafts/research-preview-release/currentness-2026-08-15/currentness-source.json",
    sha256: sourceDigest,
  ...
  for (const claim of record.claims) {
    ...
    const relativeRawPath = `drafts/research-preview-release/currentness-2026-08-15/claims/${transition.toRecordId}/${publicSafeFileSlug}.json`;
...
const urlAuditText = await readFile(urlAuditPath, "utf8");
const urlAudit = JSON.parse(urlAuditText);
const existingReceiptText = await readFile(currentnessReceiptPath, "utf8");
const freshnessCensusText = await readFile(freshnessCensusPath, "utf8");
...
assert.equal(basePreview.asOf, "2026-08-13", "Currentness refresh must start from the accepted 2026-08-13 projection");
assert.equal(basePreview.counts.surfaces, 55);
assert.equal(basePreview.counts.recordsPresentedIncludingHistory, 88);
...
assert(surfaceAudit.observations.every((item) => item.result === "rendered"));
assert.deepEqual(
...
for (const transition of source.transitions) {
  ...
  const relativeRecordPath = `drafts/research-preview-release/currentness-2026-08-15/records/${transition.toRecordId}.json`;
...
preview.provenance.currentness20260815Path = "drafts/research-preview-release/currentness-2026-08-15/currentness-source.json";
preview.provenance.currentness20260815Sha256 = sourceDigest;
preview.provenance.currentness20260815ReceiptPath = "drafts/research-preview-release/currentness-2026-08-15/currentness-receipt.json";
preview.provenance.currentness20260815OfficialSourceAuditPath = "drafts/research-preview-release/currentness-2026-08-15/official-source-audit.json";
preview.provenance.currentness20260815OfficialSourceAuditSha256 = sha256(surfaceAuditText);
preview.provenance.currentness20260815OfficialUrlAuditPath = "drafts/research-preview-release/currentness-2026-08-15/official-url-audit.json";
preview.provenance.currentness20260815OfficialUrlAuditSha256 = sha256(urlAuditText);
preview.provenance.publicationFreshnessCensusPath = "drafts/research-preview-release/currentness-2026-08-15/publication-freshness-census.json";
...
const reviewedSurfaces = surfaces.map((surface) => {
  ...
  return {
    ...
    renderedAt: observation.finalUrl,
...
const receipt = {
  ...
  officialSurfaceAudit: {
    path: "drafts/research-preview-release/currentness-2026-08-15/official-source-audit.json",
    sha256: sha256(surfaceAuditText),
    observations: surfaceAudit.observations.length,
    rendered: surfaceAudit.observations.filter((item) => item.result === "rendered").length,
    failed: surfaceAudit.observations.filter((item) => item.result !== "rendered").length,
