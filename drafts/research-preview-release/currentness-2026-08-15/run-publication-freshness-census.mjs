...
const liveListDetectors = new Map([
  ["com.cursor.ide.foreground-agent.desktop-stable", {
    ...
    preferredNewerIdentity: "3.16",
    patterns: [
      /["']version["']\s*:\s*["'](3\.\d+(?:\.\d+)?)["']/gi,
      /\bVersion\s+(3\.\d+(?:\.\d+)?)/gi,
      /\bCursor(?:[-_ ]Setup)?[-_ ](3\.\d+(?:\.\d+)?)/gi
    ]
  }],
...
function classify(reviewed, result) {
  ...
  const newer = detector.preferredNewerIdentity && newerCandidates.includes(detector.preferredNewerIdentity)
    ? detector.preferredNewerIdentity
    : newerCandidates.at(-1) ?? null;
  if (newer) {
    return {
      ...common,
