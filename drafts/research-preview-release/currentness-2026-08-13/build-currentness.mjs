...
const initialReviewedAt = "2026-08-13";
const initialCheckedAt = "2026-08-13T10:02:56.587Z";
...
async function buildSuccessor(predecessor, transition, sourceDigest) {
  ...
  const recheckAfter = transition.recheckAfter ?? "2026-09-13";
  const replacements = [
  ...
  record.sourceDossier = {
    path: "drafts/research-preview-release/currentness-2026-08-13/currentness-source.json",
    sha256: sourceDigest,
  ...
  for (const claim of record.claims) {
    ...
    const relativeRawPath = `drafts/research-preview-release/currentness-2026-08-13/claims/${transition.toRecordId}/${publicSafeFileSlug}.json`;
...
assert.equal(basePreview.asOf, "2026-08-10", "Currentness refresh must start from the accepted 2026-08-10 projection");
assert.equal(basePreview.counts.surfaces, 55);
assert.equal(basePreview.counts.recordsPresentedIncludingHistory, 73);
...
for (const transition of source.transitions) {
  ...
  const relativeRecordPath = `drafts/research-preview-release/currentness-2026-08-13/records/${transition.toRecordId}.json`;
...
preview.provenance.currentness20260813Path = "drafts/research-preview-release/currentness-2026-08-13/currentness-source.json";
preview.provenance.currentness20260813Sha256 = sourceDigest;
preview.provenance.currentness20260813ReceiptPath = "drafts/research-preview-release/currentness-2026-08-13/currentness-receipt.json";
preview.counts = {
