...
assert.equal(source.asOf, "2026-08-18");
assert.equal(surfaceAudit.asOf, "2026-08-18");
...
assert.equal(source.sourceLinkAudit.recordsChecked, 106);
...
assert.equal(source.transitions.length, 5);
...
assert.equal(preview.provenance.currentness20260818Sha256, sha256(sourceText));
assert.equal(preview.provenance.currentness20260818OfficialSourceAuditSha256, sha256(surfaceAuditText));
assert.equal(preview.provenance.currentness20260818OfficialUrlAuditSha256, sha256(urlAuditText));
...
assert.equal(new Set(preview.previewRecords.map((record) => record.recordId)).size, 106);
...
assert.equal(snapshotSeal.asOf, "2026-08-18");
...
assert.equal(lifecycle.entries.filter((entry) => entry.status === "superseded").length, 50);
...
assert(lifecycle.entries.every((entry) => entry.reviewedAt === "2026-08-18"));
...
for (const transition of source.transitions) {
  ...
  assert.equal(summary.reviewedAt, transition.reviewedAt ?? "2026-08-18");
  ...
  for (const claim of record.claims) {
    ...
    assert.equal(raw.review.reviewedAt, transition.reviewedAt ?? "2026-08-18");
    assert.equal(raw.review.recheckAfter, transition.recheckAfter ?? "2026-09-18");
    assert.equal(raw.source.capturedAt, transition.checkedAt ?? "2026-08-18T00:00:00.000Z");
...
for (const surfaceKey of source.unchangedSurfaceKeys) {
  ...
  assert(records.every((record) => record.reviewedAt === "2026-08-18"));
}
...
assert.equal(receipt.scope.priorRecordsRetained, 101);
assert.equal(receipt.scope.exactIdentitySuccessorsAdded, 5);
assert.equal(receipt.scope.recordsPresentedIncludingHistory, 106);
...
assert.equal(receipt.materialTransitions.length, 5);
assert.equal(receipt.scope.independentEvidenceCredit, 0);

console.log("PASS all 55 accepted surfaces retain their 2026-08-18 official-source decision");
console.log("PASS publication freshness chronology rejects a census that begins before the sealed snapshot");
console.log("PASS 5 exact-identity successors preserve all 101 predecessor records with reciprocal lifecycle links");
console.log("PASS 106 projected records contain 53 current and 53 retained-history records with zero independent-test credit");
