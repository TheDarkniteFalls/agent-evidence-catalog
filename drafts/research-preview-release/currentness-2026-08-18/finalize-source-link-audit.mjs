...
assert.equal(audit.asOf, "2026-08-18");
assert.equal(audit.counts.recordsChecked, 106);
...
source.sourceLinkAudit = {
  ...
  method: "HTTP GET with redirects against every unique named official source URL in the projected 106-record corpus",
...
console.log(`PASS bound ${audit.counts.uniqueOfficialUrlsChecked} projected official URLs to the 106-record currentness source (${audit.counts.unreachable} unresolved)`);
