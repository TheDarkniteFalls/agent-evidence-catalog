...
const seal = await readJson("drafts/research-preview-release/currentness-2026-08-18/snapshot-seal.json");
const census = await readJson("drafts/research-preview-release/currentness-2026-08-18/publication-freshness-census.json");
const surfaceAuditText = await read("drafts/research-preview-release/currentness-2026-08-18/official-source-audit.json");
const surfaceAudit = JSON.parse(surfaceAuditText);
const urlAuditText = await read("drafts/research-preview-release/currentness-2026-08-18/official-url-audit.json");
...
const receipt = {
  ...
  snapshot: {
    ...
    bannerCopy: "Catalog snapshot: 18 August 2026, 10:34 UTC. Agent releases change quickly; records with known updates are marked.",
    cacheBustingVersion: "2026-08-18-sealed-snapshot"
  },
  publicationAuthorQa: {
    workstream: "AEC-AGENT-LANDSCAPE-AND-ROADMAP-01-AUTHOR",
    result: "PASS",
    checkedAt,
    baseHead: "f7cdb0dbbc85a3760a944d91d5cc3dce6e3c0bb6",
    browser: "Codex in-app Browser",
    currentness: {
      records: 106,
      currentRecords: 53,
  ...
  journeys: {
    ...
    comparison: {
      result: "PASS",
      representativePair: ["com.anthropic.claude-code.cli.2-1-234", "com.openai.codex.cli.0-147-0"],
