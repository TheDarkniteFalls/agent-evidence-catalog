...
assert.equal(surfaceAudit.asOf, "2026-08-20");
...
assert.equal(source.sourceLinkAudit.recordsChecked, 115);
...
assert.equal(source.transitions.length, 9);
...
assert.equal(preview.provenance.currentness20260820Sha256, sha256(sourceText));
assert.equal(preview.provenance.currentness20260820OfficialSourceAuditSha256, sha256(surfaceAuditText));
assert.equal(preview.provenance.currentness20260820OfficialUrlAuditSha256, sha256(urlAuditText));
...
assert.equal(snapshotSeal.asOf, "2026-08-20");
...
assert.equal(lifecycle.entries.filter((entry) => entry.status === "superseded").length, 59);
...
assert(lifecycle.entries.every((entry) => entry.reviewedAt === "2026-08-20"));
...
for (const transition of source.transitions) {
  ...
  assert.equal(summary.reviewedAt, transition.reviewedAt ?? "2026-08-20");
  ...
  for (const claim of record.claims) {
    ...
    assert.equal(raw.review.reviewedAt, transition.reviewedAt ?? "2026-08-20");
    assert.equal(raw.review.recheckAfter, transition.recheckAfter ?? "2026-09-20");
    assert.equal(raw.source.capturedAt, transition.checkedAt ?? "2026-08-20T00:00:00.000Z");
...
for (const surfaceKey of source.unchangedSurfaceKeys) {
  ...
  assert(records.every((record) => record.reviewedAt === "2026-08-20"));
}
...
assert.equal(receipt.scope.priorRecordsRetained, 106);
assert.equal(receipt.scope.exactIdentitySuccessorsAdded, 9);
assert.equal(receipt.scope.recordsPresentedIncludingHistory, 115);
...
assert.equal(receipt.materialTransitions.length, 9);
...
assert(receipt.reviewedSurfaces.filter((item) => item.result === "successor-added").length === 9);
assert(receipt.reviewedSurfaces.filter((item) => item.result === "retained-no-exact-successor-admitted").length === 46);

console.log("PASS all 55 accepted surfaces retain their 2026-08-20 official-source decision");
console.log("PASS publication freshness chronology rejects a census that begins before the sealed snapshot");
console.log("PASS 9 exact-identity successors preserve all 106 predecessor records with reciprocal lifecycle links");
console.log("PASS 115 projected records contain 53 current and 62 retained-history records with zero independent-test credit");
