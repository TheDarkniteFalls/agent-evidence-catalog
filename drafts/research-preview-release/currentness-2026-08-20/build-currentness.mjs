...
const initialReviewedAt = "2026-08-20";
const initialCheckedAt = "2026-08-20T00:00:00.000Z";
...
async function buildSuccessor(predecessor, transition, sourceDigest) {
  ...
  record.sourceDossier = {
    path: "drafts/research-preview-release/currentness-2026-08-20/currentness-source.json",
    sha256: sourceDigest,
  ...
  for (const claim of record.claims) {
    ...
    const relativeRawPath = `drafts/research-preview-release/currentness-2026-08-20/claims/${transition.toRecordId}/${publicSafeFileSlug}.json`;
...
assert(
  (basePreview.asOf === "2026-08-18" && basePreview.counts.recordsPresentedIncludingHistory === priorRecordCount)
    || preparedRerun,
  "Currentness refresh must start from the published 2026-08-18 projection or its own prepared output"
);
...
for (const summary of basePreview.previewRecords.filter((record) => !successorRecordIds.has(record.recordId))) {
  ...
  if (removedLimitations > 0 || summary.recordPath.startsWith("drafts/research-preview-release/currentness-2026-08-20/records/")) {
...
for (const record of retainedProjectionCopies) {
  ...
  const relativeRecordPath = `drafts/research-preview-release/currentness-2026-08-20/records/${recordId}.json`;
...
for (const transition of source.transitions) {
  ...
  const relativeRecordPath = `drafts/research-preview-release/currentness-2026-08-20/records/${transition.toRecordId}.json`;
...
preview.provenance.currentness20260820Path = "drafts/research-preview-release/currentness-2026-08-20/currentness-source.json";
preview.provenance.currentness20260820Sha256 = sourceDigest;
preview.provenance.currentness20260820ReceiptPath = "drafts/research-preview-release/currentness-2026-08-20/currentness-receipt.json";
preview.provenance.currentness20260820OfficialSourceAuditPath = "drafts/research-preview-release/currentness-2026-08-20/official-source-audit.json";
preview.provenance.currentness20260820OfficialSourceAuditSha256 = sha256(surfaceAuditText);
if (prepareOnly) {
  delete preview.provenance.currentness20260820OfficialUrlAuditPath;
  delete preview.provenance.currentness20260820OfficialUrlAuditSha256;
  delete preview.provenance.publicationFreshnessCensusPath;
  delete preview.provenance.publicationFreshnessCensusSha256;
} else {
  preview.provenance.currentness20260820OfficialUrlAuditPath = "drafts/research-preview-release/currentness-2026-08-20/official-url-audit.json";
  preview.provenance.currentness20260820OfficialUrlAuditSha256 = sha256(urlAuditText);
  preview.provenance.publicationFreshnessCensusPath = "drafts/research-preview-release/currentness-2026-08-20/publication-freshness-census.json";
...
const receipt = {
  ...
  officialSurfaceAudit: {
    path: "drafts/research-preview-release/currentness-2026-08-20/official-source-audit.json",
