...
const initialReviewedAt = "2026-08-21";
const initialCheckedAt = "2026-08-21T00:00:00.000Z";
...
function normalizeSuccessorBoundaryLimitations(record) {
  assert(Array.isArray(record.dossier?.limitations), `${record.identity.recordId} has no dossier limitation list`);
  const seen = new Set();
  const before = record.dossier.limitations.length;
  record.dossier.limitations = record.dossier.limitations.filter((item) => {
    if (!successorBoundaryLimitations.includes(item)) return true;
    if (seen.has(item)) return false;
    seen.add(item);
    return true;
  });
  return before - record.dossier.limitations.length;
}
...
async function buildSuccessor(predecessor, transition, sourceDigest) {
  ...
  record.sourceDossier = {
    path: "drafts/research-preview-release/currentness-2026-08-21/currentness-source.json",
    sha256: sourceDigest,
  ...
  for (const claim of record.claims) {
    ...
    const relativeRawPath = `drafts/research-preview-release/currentness-2026-08-21/claims/${transition.toRecordId}/${publicSafeFileSlug}.json`;
  ...
  record.dossier.limitations = [
    ...record.dossier.limitations.filter((item) =>
      ...
      && !successorBoundaryLimitations.includes(item)
    ),
    ...successorBoundaryLimitations
  ];
...
const priorRecordCount = 115;
const successorRecordIds = new Set(source.transitions.map((transition) => transition.toRecordId));
const successorSourceIds = new Set(source.transitions.flatMap((transition) => transition.basisSourceIds));
const preparedRerun = basePreview.asOf === source.asOf
  && basePreview.counts.recordsPresentedIncludingHistory === priorRecordCount + source.transitions.length;
assert(
  (basePreview.asOf === "2026-08-20" && basePreview.counts.recordsPresentedIncludingHistory === priorRecordCount)
    || preparedRerun,
  "Currentness refresh must start from the published 2026-08-20 projection or its own prepared output"
);
assert.equal(basePreview.counts.surfaces, 55);
if (preparedRerun) {
  lifecycle.entries = lifecycle.entries.filter((entry) => !successorRecordIds.has(entry.recordId));
  lifecycle.sources = lifecycle.sources.filter((item) => !successorSourceIds.has(item.id));
  ...
  for (const transition of source.transitions) {
    ...
    assert(predecessor, `Prepared rerun is missing predecessor ${transition.fromRecordId}`);
    predecessor.status = "current";
    predecessor.supersededByRecordId = null;
  }
}
...
const retainedProjectionCopies = [];
for (const summary of basePreview.previewRecords.filter((record) => !successorRecordIds.has(record.recordId))) {
  const record = await readJson(path.join(packageRoot, summary.recordPath));
  const removedLimitations = normalizeSuccessorBoundaryLimitations(record);
  if (removedLimitations > 0 || summary.recordPath.startsWith("drafts/research-preview-release/currentness-2026-08-21/records/")) {
    retainedProjectionCopies.push(record);
  }
  recordsById.set(summary.recordId, record);
...
for (const record of retainedProjectionCopies) {
  const recordId = record.identity.recordId;
  const relativeRecordPath = `drafts/research-preview-release/currentness-2026-08-21/records/${recordId}.json`;
  await writeFile(path.join(packageRoot, relativeRecordPath), serialize(record));
  recordPathsById.set(recordId, relativeRecordPath);
}
...
for (const transition of source.transitions) {
  ...
  const relativeRecordPath = `drafts/research-preview-release/currentness-2026-08-21/records/${transition.toRecordId}.json`;
...
for (const record of recordsById.values()) {
  for (const limitation of successorBoundaryLimitations) {
    assert(
      countExactString(record, limitation) <= 1,
      `${record.identity.recordId} repeats successor-boundary limitation: ${limitation}`
    );
  }
}

lifecycle.asOf = source.asOf;
lifecycle.interpretationBoundary.note = "This additive all-surface currentness projection preserves every prior record, rechecks retention state for all 55 surfaces, and adds only official-source exact-identity successors. A retained surface means no exact successor was admitted; it does not claim that rolling source prose was unchanged. The projection does not add observed behaviour, independent evidence, scoring, ranking, recommendations or suitability calculations.";
...
preview.provenance.currentness20260821Path = "drafts/research-preview-release/currentness-2026-08-21/currentness-source.json";
preview.provenance.currentness20260821Sha256 = sourceDigest;
preview.provenance.currentness20260821ReceiptPath = "drafts/research-preview-release/currentness-2026-08-21/currentness-receipt.json";
preview.provenance.currentness20260821OfficialSourceAuditPath = "drafts/research-preview-release/currentness-2026-08-21/official-source-audit.json";
preview.provenance.currentness20260821OfficialSourceAuditSha256 = sha256(surfaceAuditText);
if (prepareOnly) {
  delete preview.provenance.currentness20260821OfficialUrlAuditPath;
  delete preview.provenance.currentness20260821OfficialUrlAuditSha256;
  delete preview.provenance.publicationFreshnessCensusPath;
  delete preview.provenance.publicationFreshnessCensusSha256;
} else {
  preview.provenance.currentness20260821OfficialUrlAuditPath = "drafts/research-preview-release/currentness-2026-08-21/official-url-audit.json";
  preview.provenance.currentness20260821OfficialUrlAuditSha256 = sha256(urlAuditText);
  preview.provenance.publicationFreshnessCensusPath = "drafts/research-preview-release/currentness-2026-08-21/publication-freshness-census.json";
...
const reviewedSurfaces = surfaces.map((surface) => {
  ...
  return {
    ...
    result: transition ? "successor-added" : "retained-no-exact-successor-admitted",
...
const receipt = {
  ...
  scope: {
    ...
    priorRecordsRetained: priorRecordCount,
  ...
  officialSurfaceAudit: {
    path: "drafts/research-preview-release/currentness-2026-08-21/official-source-audit.json",
  ...
  sourceOnlyDossierDecisions: source.sourceOnlyDossierDecisions,
  excludedScopeDecisions: source.excludedScopeDecisions,
...
const markdown = `# 55-surface currentness receipt\n\nChecked: all 55 surfaces on ${source.asOf} against official publisher sources.\n\nResult: **PASS**. Every accepted surface was rechecked. ${source.transitions.length} newer exact identities were added as same-surface successors; all ${priorRecordCount} prior records remain inspectable. No agent was installed or run, no independent evidence was credited, and no score, ranking, recommendation or suitability calculation was added.\n\n## Surface review\n\n| # | Surface | Current identity | Result |\n|---:|---|---|---|\n${rows}\n\n## Exact-identity transitions\n\n| Surface | Preserved predecessor | Current successor | Official source |\n|---|---|---|---|\n${transitions}\n\n## Boundaries\n\nAll ${surfaceAudit.observations.length} preferred surface-level official publisher sources were reachable during the bounded refresh check. The ${source.sourceLinkAudit.uniqueOfficialUrlsChecked} unique official-source URLs projected across all ${source.sourceLinkAudit.recordsChecked} records were also audited for reachability; ${source.sourceLinkAudit.reachable} were reachable and ${source.sourceLinkAudit.unreachable} were not. Reachability and response capture are not product observation or independent verification. A retained surface means no exact successor was admitted; it does not claim rolling source prose was unchanged. Cursor CLI, Windsurf Cascade, Copilot Agent Mode for Visual Studio and Zoo Code remain source-only and non-admitted. CodeRabbit, Greptile and a generic JetBrains agent-host surface remain excluded pending a separate scope decision. Rolling runtime, model, configuration, authority, installation and service-revision applicability remain unresolved where the records say so.\n`;
...
console.log(`PASS preserved ${priorRecordCount} prior records and built ${previewRecords.length} total record pages`);
