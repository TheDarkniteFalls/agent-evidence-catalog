...
${publicationFreshnessNotice ? `        ${publicationFreshnessNotice}\n` : ""}        <div class="detail-actions">
    ...
    <script src="../data.js?v=2026-08-18-sealed-snapshot"></script>
    <script src="../comparison-core.js?v=2026-08-16-visitor-ia-1"></script>
    <script src="../record-detail.js?v=2026-08-18-sealed-snapshot"></script>
  </body>
</html>
`;
}
...
async function commandBuild() {
  ...
  const snapshotSealSource = join(ROOT, "drafts", "research-preview-release", "currentness-2026-08-18", "snapshot-seal.json");
  const snapshotSealRaw = await readFile(snapshotSealSource, "utf8");
  const snapshotSeal = JSON.parse(snapshotSealRaw);
  const freshnessCensusSource = join(ROOT, "drafts", "research-preview-release", "currentness-2026-08-18", "publication-freshness-census.json");
